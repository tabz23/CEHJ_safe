...
- Panel video per episode attempt (warmup: all; rounds: filter episodes only).
  Observer frame + fixed-height panel; the bbox overlay
  (obstacle/held/link collision boxes) is OFF by default — set
  `cfg.video_bbox = True` (or edit config.py) for debug runs. Panel:
...
- Encoder: frozen X-VLA VLM (`network/encoder.py` XVLAEncoder; ckpt dir from
  `XVLA_CKPT`, default `/root/autodl-tmp/XVLA_ckpt/robotwin2`). The ckpt's
  vendored Florence-2 code needs two runtime shims for transformers 4.57
...
- a_nom is the EE6D delta from the LIVE measured EE pose to the plan-row EE
  ...
  `_calibrate_ee`).
- Buffer (`train/buffer.py`) is slim: scene_tokens [200,1024] fp16
  (PRE-adapter — the 1024→256 adapter TRAINS at train time, so its inputs
  are stored), proprio [20], action [18], h, ids, done. No
  Jacobians/padding (EE6D is embodiment-agnostic).
- Trainable: actor, twin critics (EE6DTrunk cross-attn), the scene-token
  adapter, proprio_enc (Polyak target alongside the critics). quat
  convention: TRUE wxyz rotations everywhere (X-VLA's own client
  round-trips through a self-cancelling xyzw/wxyz permutation; the frozen
  VLM never sees proprio, so we don't replicate the permutation).
