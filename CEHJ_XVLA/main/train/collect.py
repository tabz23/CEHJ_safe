"""Stage 1 — collection driver for the X-VLA joint-space HJ-SAC baseline.
...
tick at 25 Hz the frozen X-VLA scene tokens, the EE-pose proprio, the
analytic body features (Jlin/Jang/joint_index/dtheta_max), and the
privileged h are captured; the dtheta stored per entry is the COMMANDED
joint displacement of that tick (planner a_nom / actor / perturbation —
see action_source).
...
Usage:
    python -m main.train.collect            (from CEHJ_XVLA/)
...
CKPT_DIR = Path(
    os.environ.get("XVLA_CKPT", "/root/autodl-tmp/XVLA_ckpt/robotwin2")
)
...
def _load_follow_models(ckpt_path: Path, built: dict | None = None,
    ...
    the --follow side of async online collection. Also refreshes the
    encoder's learned adapter / proprio_enc when an encoder is given."""
    from main.network.heads import JointActor, JointTwinCritic
    ...
    if built is None:
        ...
        built = {
            "actor": JointActor().cuda().eval(),
            "critics": JointTwinCritic(temperature=T).cuda().eval(),
        }
    ...
    if encoder is not None:
        encoder.adapter.load_state_dict(ckpt["adapter"])
        encoder.proprio_enc.load_state_dict(ckpt["proprio_enc"])
    return built["actor"], built["critics"], built
...
def collect(cfg: FrozenConfig, out_root: Path, buf: StepBuffer | None = None,
    ...
    X-VLA tokens + EE-pose proprio + body features + h are written to the
    buffer; the action is the commanded dtheta of that tick.
    ...
    if encoder is None:
        encoder = XVLAEncoder(str(CKPT_DIR), device="cuda")

    if buf is None:
        # buffer_dir override: the memmap is the training hot path — keep it
        # on LOCAL disk, not a network FS
    ...
    while True:
        ...
        if follow_path is not None:
            if follow_path.exists():
                ...
                if mt != follow_mtime:
                    try:
                        actor, critics, built = _load_follow_models(
                            follow_path, built, cfg, encoder=encoder
                        )
                        models = (actor, critics)
                        follow_mtime = mt
        ...
        for _retry in range(5):
            ...
            try:
                candidate = RolloutController(
                    ...
                    encoder=encoder,
                    actor=models[0] if models else None,
                    critics=models[1] if models else None,
        ...
        if success_only and not success:
            ...
            # its schedule coverage (some combos are unreachable)
