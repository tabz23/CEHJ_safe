"""Stage 1 — collection driver for the X-VLA EE6D HJ-SAC baseline.
...
tick at 25 Hz the frozen X-VLA scene tokens, the EE6D proprio, and the
privileged h are captured; the action stored per entry is the COMMANDED
EE6D delta of that tick (planner a_nom / actor / perturbation — see
action_source).
...
# kinematics URDFs only (FK for a_nom) — the HoloBrain checkpoint dir holds
# the dual-arm URDFs; nothing HoloBrain is loaded into the models
KIN_URDF_DIR = Path(
...
def make_kinematics(ckpt_dir: Path, embodiment: str = "piper"):
    """MultiArmKinematics for the given embodiment (FK for a_nom EE targets).
    ...
    if embodiment == "aloha-agilex":
        # the robotwin2_0 processor uses DualArmKinematics on this exact
        # URDF (fl_/fr_ links); defaults are the aloha layout (arm joints
        # 10-15/18-23, fingers merged per arm)
...
def _load_follow_models(ckpt_path: Path, built: dict | None = None,
    ...
    if built is None:
        ...
        built = {
            "actor": EE6DActor().cuda().eval(),
            "critics": EE6DTwinCritic(temperature=T).cuda().eval(),
        }
...
def collect(cfg: FrozenConfig, out_root: Path, buf: StepBuffer | None = None,
    ...
    X-VLA tokens + EE6D proprio + h are written to the buffer; the action
    is the commanded EE6D delta of that tick.
    ...
    while True:
        ...
        for _retry in range(5):
            ...
            if cfg_ep.embodiment not in kins:
                kins[cfg_ep.embodiment] = make_kinematics(
                    KIN_URDF_DIR, cfg_ep.embodiment)
