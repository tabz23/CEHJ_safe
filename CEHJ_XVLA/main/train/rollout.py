...
X-VLA variant of the joint-space HJ-SAC rollout — SAME action space as the
parent: per-tick joint displacements dtheta, NOT EE6D. The task's
play_once owns the stage machine. TickChunkedController walks each cuRobo
plan in fixed tick chunks (exactly PHYSICS_FREQ/control_freq physics steps
— 10 at 25 Hz) and calls the per-tick hook at every boundary:
...
Filter trigger: Q(s, a_nom) < margin — the critic scores the NOMINAL's
action for the upcoming tick (a_nom = pos[min(i_end, T-1)] - qpos_now, per
arm, referenced to the LIVE qpos), which is the textbook least-restrictive
semantics.

Action space (same as the parent): per arm the spec's [j1..j7] slots
(franka 7-DoF; piper/6-DoF arms drop the 7th slot) followed by the 2
prismatic gripper slots — the filter never drives the gripper (the task
script owns it); those columns stay exactly zero. dtheta is bounded by
dtheta_max = URDF velocity limit x control_dt x kappa (kappa = 1.0, from
BodyTokenExtractor) and executed via env.step_dtheta — the parent's
per-tick velocity-feedforward stepping (interpolated position targets
across the 10 physics substeps), so actor ticks, planner ticks, and
perturbations all share the tick structure.

Representation: X-VLA has NO robot-state encoder. The buffer stores the
PRE-adapter frozen VLM scene tokens ([200, 1024]) and the raw EE-pose
proprio [20] (per arm [xyz, rot6d, 1-2g]); the learned adapter/proprio_enc
run at train time. The filter consumes the adapted tokens (arm_tokens +
scene) via the enc namespace.
...
and execution. The proprio uses the TRUE rotation (wxyz); the frozen VLM
never sees proprio (only our learned adapter does), so there is no parity
constraint with X-VLA's permuted convention.
...
scene.step). The buffer's dtheta is the COMMANDED action per tick (planner
a_nom, actor dtheta, or the sampled perturbation), with action_source
marking which.
"""
...
from main.network.heads import arm_layout  # noqa: E402
...
class RolloutController:
    ...
    def __init__(self, cfg, seed, mode="filtered", encoder=None,
        ...
        if actor is not None and critics is not None:
            ...
            cols, ee = arm_layout(
                ext["joint_index"].numpy(), ext["body_mask"].numpy()
            )
    ...
    def _ee_state(self) -> dict:
        ...
        (get_xvla_obs renders 3 cameras; proprio needs only the sim state).
        Same dict keys minus the images."""
    ...
    def _proprio(self, obs) -> np.ndarray:
        """X-VLA EE-pose proprio [20]: per arm [xyz, rot6d(wxyz quat),
        1-2*g]. (Input encoding only — includes the gripper; the ACTION
        does not. Kept from the EE6D baseline: X-VLA has no robot-state
        encoder, so this is the policy's only robot-state channel.)"""
    ...
    def _tick_hook(self, ctx) -> bool:
        ...
        # hysteresis: engaged holds until Q clears the release margin;
        # free nominal engages only below the (tighter) engage margin
    ...
    def _capture_tick(self):
        ...
        if self.flt is not None and self.filter_active:
            self._obs_cache = (self._n_physics, encoded[3], encoded[2])
        else:
            self._obs_cache = None
    ...
    def _collect_tick(self, obs, h, encoded):
        ...
        COMMANDED dtheta of the tick that led here (planner a_nom, actor
        ...
        buffer maxima (action side 18). Jlin/Jang and dtheta_max are stored
        per step — a mixed-embodiment buffer cannot recompute them from qpos
        at sample time."""
        from main.train.buffer import MAX_ACTION, N_BODY_TOKENS

        # encoding comes from _capture_tick (computed once per tick)
        feats, proprio, _enc, body = encoded
        ...
        Jlin_pad = np.zeros((N_BODY_TOKENS, 3, MAX_ACTION), np.float16)
        Jang_pad = np.zeros((N_BODY_TOKENS, 3, MAX_ACTION), np.float16)
    ...
    def _policy_inputs_cached(self):
        ...
        enc, body = self._encode_obs(self.env.get_xvla_obs())[2:4]
        return body, enc

    def _encode_obs(self, obs):
        """obs -> (scene_raw [1,200,1024], proprio np [20], enc namespace,
        body) — the frozen X-VLA VLM features, the raw proprio, the policy
        encoding (learned adapter + proprio_enc applied), and the analytic
        body features (Jlin/Jang/joint_index). Runs ONCE per tick; the
        buffer stores the RAW features/proprio (adapter and proprio_enc
        train at train time), the filter consumes the adapted tokens (see
        _capture_tick)."""
        with torch.no_grad():
