...
Annealing: gamma -> 1 (so Q doesn't collapse onto the h shortcut — watch
the h - V gap open), alpha -> 0 aggressively (the entropy bonus is
unsafe-optimistic: soft value >= max_a Q, and higher Q means safer).
...
class Trainer:
    def __init__(self, cfg: FrozenConfig, data_root: Path, run_dir: Path):
        ...
        self.buf = StepBuffer(Path(data_root) / "buffer", capacity=10**9)
        ...
        for p in self.critics_targ.parameters():
    ...
    # ---------- batch ----------
    def _encode(self, b: dict, suffix: str = ""):
    ...
    def grad_step(self) -> dict:
        ...
        enc_n, Jlin_n, Jang_n, ji_n = self._encode(b, "_next")
        h = _t(b["h"])                                    # [B]
        ...
        with torch.no_grad():
            for tp, sp in zip(self.critics_targ.parameters(), self.critics.parameters()):
        ...
        return {
            ...
            "mean_q": float(q1.mean()), "mean_h": float(h.mean()),
            "mean_gap_h_q": float((h - q1).mean()),
